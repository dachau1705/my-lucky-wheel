...
export default function App() {
  ...
  const [prizes, setPrizes] = useState([
    { order: 1, name: "Giải Ba", quantity: 3 },
  ...
  //  SPIN SCREEN
  // =======================
  const [digitCount, setDigitCount] = useState(maxNumber.toString().length);
  useEffect(() => {
  ...
  const spin = () => {
    ...
    do {
      finalCode = Array.from({ length: digitCount })
        .map(() => randomDigit()) // randomDigit có thể vẫn là 0–9
        .join("");
    } while (
      existingCodes.includes(finalCode) || // không trùng
      Number(finalCode) < Number(minNumber) || // < min
      Number(finalCode) > Number(maxNumber) // > max
    );
      ...
      <div style={{ width: "100vw" }}>
        <div style={containerStyle}>
          <div style={cardStyle}>
            ...
            {prizes.map((p, i) => (
              <div key={i} style={{ marginBottom: "12px" }}>
                ...
                <input
                  ...
                  value={p.order}
                  onChange={(e) =>
                    updatePrize(i, "order", Number(e.target.value))
                  }
                ...
                <input
                  ...
                  value={p.quantity}
                  onChange={(e) =>
                    updatePrize(i, "quantity", Number(e.target.value))
                  }
                  style={{ ...inputStyle, width: "60px" }}
                />
              </div>
            ...
            <button onClick={addPrize} style={buttonSecondary}>
              ➕ Thêm giải
            </button>
